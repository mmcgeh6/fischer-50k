...
All use-type square footage fields as NUMERIC columns. Use snake_case naming derived from the use type name. The full list of use types from the LL84 dataset (convert each to snake_case + `_sqft` suffix):
...
Clean up test row after verification.
  ...
  - Building_Metrics table created in Supabase with BBL primary key, 11 base fields, 60 use-type sqft columns, timestamps
