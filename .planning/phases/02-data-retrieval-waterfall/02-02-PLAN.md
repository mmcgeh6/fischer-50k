...
must_haves:
  truths:
    ...
    - "PLUTO API fetches building structure data as fallback"
...
Then test PLUTO: `python -c "from lib.nyc_apis import call_pluto_api; result = call_pluto_api('1001580001'); print(f'Year built: {result.get(\"year_built\", \"N/A\")}') if result else print('No PLUTO data')"` — should return building data.
  </verify>
  <done>
  ...
  - PLUTO API returns building structure data as fallback
...
- PLUTO returns year_built/gfa for known BBL
...
- PLUTO validates BBL format before querying
