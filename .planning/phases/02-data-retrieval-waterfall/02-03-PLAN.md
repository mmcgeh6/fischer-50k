...
must_haves:
  truths:
    ...
    - "System falls back to GeoSearch if BBL not in LL97 table"
...
- If found: Extract BBL, BIN, canonical address, zip_code, compliance pathway
- If NOT found: Call `call_geosearch_api()` using the BBL's known address (this is the fallback per DATA-02). For GeoSearch fallback, we need an address — if we only have BBL, call PLUTO first to get an address, then GeoSearch. If GeoSearch also fails, raise a descriptive error.
- Track data source: `'ll97'` or `'geosearch'`

**Step 2: Live Usage Fetch** (DATA-03, DATA-05, DATA-06, DATA-07)
- Call `call_ll84_api(bin_number)` using BIN from Step 1
- If LL84 returns data: merge energy metrics + all use-type sqft fields into result dict
- If LL84 returns None: Call `call_pluto_api(bbl)` as fallback (DATA-05). PLUTO provides year_built, gfa, but NOT energy metrics (those will be None)
- Track data source: append `',ll84_api'` or `',pluto'`
...
Test database.py additions: `python -c "from lib.database import check_building_processed; ts = check_building_processed('1001580001'); print(f'Processed: {ts}')"` — should show timestamp.
  ...
  - Step 1 queries LL97 table, falls back to GeoSearch
  - Step 2 calls LL84 API, falls back to PLUTO
...
- Add `data_source` to session state so it persists across reruns
- Add `last_processed` timestamp to session state
...
- Fallback logic works (GeoSearch if no LL97, PLUTO if no LL84)
...
- System correctly falls back (GeoSearch for identity, PLUTO for energy data)
